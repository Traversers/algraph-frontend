...
const CreateGraph = () => {
  ...
  const handleNodePlacement = (event) => {
    ...
    if (nodeExists) {
      message.error(texts.texts.CreateGraphErrors.nodeExists);
      return;
    }
  ...
  const deleteNode = () => {
    if (nodes.length === 0) {
      message.error(texts.texts.CreateGraphErrors.noNodes);
      return;
    }
  ...
  const deleteEdge = () => {
    if (edges.length === 0) {
      message.error(texts.texts.CreateGraphErrors.noEdges);
      return;
    }
  ...
  const buttons = [
    {
      ...
      onClick: handleStartPlacingNode,
    },
